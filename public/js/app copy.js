document.addEventListener('DOMContentLoaded', function () {
    ...
    function activatePrediction(pointCloudFileName) {
        ...
        button.addEventListener('click', function () {
            ...
            fetch('/api/files/run-update-prediction', {
                ...
                if (!response.ok) throw new Error(`更新预测文件中${pointCloudFileName}地址运行失败`);
                ...
                const pointCloudFileName = data.pointCloudFileName; // 获取点云文件名
                // console.log(pointCloudContent);

                // 4. 显示转换成功提示
                alert(`预测成功`);
                console.log('预测成功:', data);
    ...
    function createColorBar(minValue, maxValue, isPressure, colorBarId, maxValueTitleId, minValueTitleId) {
        ...
        if (isPressure) {
            ...
            // gradient.addColorStop(0, '#0000FF'); // 蓝色（低速）
            // gradient.addColorStop(0.5, '#FFFF00'); // 黄色（中速）
            // gradient.addColorStop(1, '#FF0000'); // 红色（高速）
            // 速度的颜色映射：蓝色到绿色到红色
            gradient.addColorStop(0, '#0000FF'); // 蓝色（低速）
            gradient.addColorStop(0.5, '#00FF00'); // 绿色（中速）
            gradient.addColorStop(1, '#FF0000'); // 红色（高速）
        }
